...
def newItem():
    if request.method == 'POST':
        ...
        category = (
            session.query(Category).filter_by(name=selectedCategory).one()
        )
        user_id = login_session['user_id']
        if (
            selectedCategory and
...
# Edit Item
@app.route(
    '/catalog/categories/<int:category_id>/items/<int:item_id>/edit',
    methods=['GET', 'POST']
)
@login_required
...
# Delete Item
@app.route(
        '/catalog/categories/<int:category_id>/items/<int:item_id>/delete',
        methods=['GET', 'POST']
    )
...
@app.route((
    "/catalog/categories/<string:category_name>"
    "/items/<string:item_name>/JSON"
))
